...
MR_cc_adj = getMarginalRevenueAvgUsage(add_rec_cc_adj, sim_usage_cc_adj, 'Adj. Crossover Cable', 180);

% Multi Rack - Add.Rec: 6
add_rec_mr = 6;
sim_usage_mr = [0, 0, 0, 0.001, 0.007, 0.024, 0.086,0.307, 0.574 ];
MR_mr = getMarginalRevenueAvgUsage(add_rec_mr, sim_usage_mr, 'Multi Rack', 180);


% Lat. Pulldown - Add.Rec.: 3
add_rec_lp = 3;
sim_usage_lp = [0.004, 0.018, 0.088, 0.304, 0.586];
MR_lp = getMarginalRevenueAvgUsage(add_rec_lp, sim_usage_lp, 'Lat Pulldown', 180);

% Fixed Crossover Cable - Add.Rec.: 2
add_rec_cc_fxd = 2;
sim_usage_cc_fxd = [0.007, 0.076, 0.370, 0.548];
MR_cc_fxd =  getMarginalRevenueAvgUsage(add_rec_cc_fxd, sim_usage_cc_fxd, 'Fxd. Crossover Cable', 180);

% Abdominal - Add.Rec.: 1
add_rec_ab = 1;
sim_usage_ab = [0.3255, 0.6745];
MR_ab = getMarginalRevenueAvgUsage(add_rec_ab, sim_usage_ab, 'Abdominal', 180);


% Chest Incline - Add.Rec.: 1
add_rec_ci = 1;
sim_usage_ci = [0.3229, 0.6771];
MR_ci = getMarginalRevenueAvgUsage(add_rec_ci, sim_usage_ci, 'Chest Incline', 180);


% Chest Press - Add.Rec.: 1
add_rec_cp = 1;
sim_usage_cp = [0.1654, 0.8346];
MR_cp = getMarginalRevenueAvgUsage(add_rec_cp, sim_usage_cp, 'Chest Press', 180);

% Chest Press (PL) - Add.Rec.: 1
add_rec_cp_pl = 1;
sim_usage_cp_pl = [0.2956, 0.6107];
MR_cp_pl = getMarginalRevenueAvgUsage(add_rec_cp_pl, sim_usage_cp_pl, 'Chest Press (PL)', 180);

% Hip Thruster - Add.Rec.: 1
add_rec_ht = 1;
sim_usage_ht = [0.3073, 0.6927];
MR_ht = getMarginalRevenueAvgUsage(add_rec_ht, sim_usage_ht, 'Hip Thruster', 180);

% Incline Bench Rack - Add.Rec.: 1
add_rec_ibr = 1;
sim_usage_ibr = [0.3424, 0.4544];
MR_ibr = getMarginalRevenueAvgUsage(add_rec_ibr, sim_usage_ibr, 'Incline Bench Rack', 180);


% Leg Press (PL) - Add.Rec.: 1
add_rec_lp_pl = 1;
sim_usage_lp_pl = [0.2513, 0.7487];
MR_lp_pl = getMarginalRevenueAvgUsage(add_rec_lp_pl, sim_usage_lp_pl, 'Leg Press (PL)', 180);

% Linear Leg Press (PL) - Add.Rec.: 1
add_rec_llp_pl = 1;
sim_usage_llp_pl = [0.2708, 0.7292];
MR_llp_pl = getMarginalRevenueAvgUsage(add_rec_llp_pl, sim_usage_llp_pl, 'Linear Leg Press (PL)', 180);

% Pulldown - Add.Rec.: 1
add_rec_p = 1;
sim_usage_p = [0.1641, 0.3411, 0.4896];
MR_p = getMarginalRevenueAvgUsage(add_rec_p, sim_usage_p, 'Pulldown', 180);

% Row (PL) - Add.Rec.: 1
add_rec_r_pl = 1;
sim_usage_r_pl = [0.2539, 0.7292];
MR_r_pl = getMarginalRevenueAvgUsage(add_rec_r_pl, sim_usage_r_pl, 'Row (PL)', 180);


% Seated Row - Add.Rec.: 1
add_rec_sr = 1;
sim_usage_sr = [0.0352, 0.9648];
MR_sr = getMarginalRevenueAvgUsage(add_rec_sr, sim_usage_sr, 'Seated Row', 180);

% Smith Machine - Add.Rec.: 1
add_rec_sm = 1;
sim_usage_sm = [0.0872, 0.2734, 0.6393];
MR_sm = getMarginalRevenueAvgUsage(add_rec_sm, sim_usage_sm, 'Smith Machine', 180);

% Wide Chest Press - Add.Rec.: 1
add_rec_wcp = 1;
sim_usage_wcp = [0.3763, 0.6237];
MR_wcp = getMarginalRevenueAvgUsage(add_rec_wcp, sim_usage_wcp, 'Wide Chest Press (PL)', 180);
